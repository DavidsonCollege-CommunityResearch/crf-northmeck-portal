...
**Assets:** images live under `public/images/`. The three photographic hero/lifestyle images (`hero.jpg`, `housing-hero.jpg`, `cbr-photo.jpg`) are temporary free stock photos (Pexels license) standing in for real photography the design team hasn't delivered yet; swap them out once the real assets arrive. Two partner logos (`mulliss-logo.png`, `ada-jenkins-logo.png`) referenced in `about.astro` are still missing entirely and need the actual organization logos, not stock substitutes.
